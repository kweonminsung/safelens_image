...
class ImageGenerator:
    ...
    def generate_replacement(self, image: Image.Image, region: BoundingBox, label: str = None) -> Optional[Image.Image]:
        ...
        try:
            ...
            print(f"[DEBUG] Blacked area: ({mask_x1}, {mask_y1}) to ({mask_x2}, {mask_y2}) [padding: {self.mask_padding}px]")
                ...
                if label:
                    ...
                    if replacement_value:
                        ...
                        if "license" in label_lower or "plate" in label_lower:
                            ...
                            prompt_text += "\n\nIMPORTANT: The black area contained a face. Fill it naturally with a blurred or obscured appearance."
                        else:
    ...
    def generate_replacements_batch(
        ...
        try:
            ...
            for i, info in enumerate(region_infos):
                print(f"[DEBUG]   Region {i+1}: bbox={info['bbox']}, masked={info['masked_bbox']}, label={info['label']}")
                    ...
                    if label:
                        ...
                        if replacement_value:
                            if "license" in label_lower or "plate" in label_lower:
                                ...
                                prompt_text += f"{i}. Email - fill with '{replacement_value}'\n"
                            elif "address" in label_lower:
                                prompt_text += f"{i}. Address - fill with '{replacement_value}'\n"
                            elif "name" in label_lower:
                                prompt_text += f"{i}. Name - fill with '{replacement_value}'\n"
                            else:
                                prompt_text += f"{i}. {label_str} - fill with '{replacement_value}'\n"
                        else:
                            if "face" in label_lower:
                                prompt_text += f"{i}. Face - blur or obscure naturally\n"
                            else:
                                prompt_text += f"{i}. {label_str} - fill with natural background\n"
                    else:
                ...
                if response.parts:
                    for part in response.parts:
                        if part.inline_data:
                            ...
                            gen_img.save(debug_path)
                            print(f"[DEBUG] Saved batch generated image to {debug_path}")
                            print(f"[DEBUG] Original size: {image.size}, Generated size: {gen_img.size}")
